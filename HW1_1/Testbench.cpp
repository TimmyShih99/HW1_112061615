...
void Testbench::do_sobel() {
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
      ...
      total = i_result.read();  //註解的
      //cout << "Now at " << sc_time_stamp() << endl; //print current sc_time

      //if, else要註解  //註解的
      /*if (total - THRESHOLD >= 64) {
        // black
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = BLACK;    //BLACK
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = BLACK;
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = BLACK;
      } else {
        // white
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = WHITE;    //WHITE
